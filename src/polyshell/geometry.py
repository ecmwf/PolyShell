...
class LineString:
    ...
    def triangles2(self) -> Iterator[Triangle]:
        """Return an iterator of triangles formed of internal points and the endpoints."""
        return map(
            Triangle.__call__,
            zip(LineString([self.points[0]] * (len(self.points) - 2)),
                LineString([self.points[-1]] * (len(self.points) - 2)), self.points[1:-1]),
        )
