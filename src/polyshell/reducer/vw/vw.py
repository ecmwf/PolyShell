...
class VWLineString:
    ...
    def tree_intersect(self, triangle: VWScore) -> bool:
        ...
        bounding_rect = Triangle(
            (self.orig[triangle.left], self.orig[triangle.current], self.orig[triangle.right])
        ).bbox()
    ...
    def recompute_triangles(
        ...
        for ai, current_point, bi in choices:
            ...
            score = self.loss_fn(Triangle((self.orig[ai], self.orig[current_point], self.orig[bi])))
            if score < 0:
