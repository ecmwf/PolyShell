...
def reduce_polygon_vw(poly: Polygon, eps: float, loss_fn: LossFunction) -> Polygon:
    vw_states = init_vw_states(poly, loss_fn, VWRunningMode.Serial)
    reduce_states_vw(vw_states, eps, 2)
    return merge_states_vw(vw_states)
...
def init_vw_states(poly: Polygon, loss_fn: LossFunction, mode: VWRunningMode) -> list[VWLineString]:
...
def reduce_states_vw(vw_states: list[VWLineString], eps: float, min_sec_len: int, max_workers: Optional[int] = None):
